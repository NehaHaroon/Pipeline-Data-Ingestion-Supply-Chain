...
def _load_config() -> ConsumerConfig:
    ...
    # if any(server.startswith("kafka:") for server in bootstrap_servers):
    #     for fallback in ["localhost:9092", "127.0.0.1:9092"]:
    #         if fallback not in bootstrap_servers:
    #             bootstrap_servers.append(fallback)

    topic = os.getenv("CDC_TOPIC", "supply_chain.public.inventory_transactions")
    group_id = os.getenv("CDC_GROUP_ID", "cdc-ingestion-group")
    api_url = os.getenv(
        "INGESTION_API_URL",
        "http://localhost:8000/ingest/src_inventory_transactions",
    )
...
def _forward_to_ingestion_api(config: ConsumerConfig, envelope: EventEnvelope) -> None:
    ...
    body = {"records": [envelope.to_dict()]}
    try:
        response = requests.post(config.api_url, headers=headers, json=body, timeout=30)
        log.info(f"[CDC] API response: {response.status_code} {response.text}")
        response.raise_for_status()
    except Exception as e:
        log.info(f"[CDC] API ERROR: {e}")


def run_consumer() -> None:
    """Main loop consuming Debezium topic and forwarding to ingestion API."""
    config = _load_config()
    log.info(f"[CDC] Connecting to Kafka using bootstrap servers: {config.bootstrap_servers}")
    ...
    try:
        ...
        log.info("[CDC] Shutdown requested by user.")
    finally:
        consumer.close()
        log.info("[CDC] Consumer closed cleanly.")
