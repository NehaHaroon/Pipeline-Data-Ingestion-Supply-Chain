...
def generate_visualizations():
    """Generate matplotlib plots for each dataset if parquet files exist."""
    plots = []
    for dataset in ALL_DATASETS:
        ...
        try:
            ...
            if dataset_id == "ds_sales_history" and "sale_timestamp" in df.columns and "units_sold" in df.columns:
                ...
                plt.xlabel("Sale Timestamp"); plt.ylabel("Units Sold")
                ...
                plt.xlabel("Product ID"); plt.ylabel("Reorder Threshold")
                ...
                plt.xlabel("Manufacturing Timestamp"); plt.ylabel("Quantity Produced")
                plt.gca().xaxis.set_major_formatter(mdates.DateFormatter('%Y-%m-%d'))
                plt.xticks(rotation=45)
                ...
                plt.xlabel("Timestamp"); plt.ylabel("Current Stock on Shelf")
                ...
                plt.xlabel("Historical Period"); plt.ylabel("Total Monthly Sales")
                plt.xticks(rotation=45)
            else:
            ...
            log.error(f"Error generating plot for {dataset_id}: {e}")
    return plots
...
#     <!-- filter summary -->
#     <div class="fsummary hidden ai" id="fsummary">
#       Showing <strong id="f-cnt">—</strong> of <strong id="t-cnt">—</strong> records &nbsp;
#       <span style="color:var(--t3)" id="f-desc"></span>
#     </div>
...
# // function genMock() {
# //   const SRCS = [
# //     { source_id:'src_sales_history',      name:'Sales History',      source_type:'database',  ingestion_frequency:'daily',    domain:'sales',      classification_level:'internal',      schema_version:'v2' },
# //     { source_id:'src_warehouse_master',   name:'Warehouse Master',   source_type:'batch',     ingestion_frequency:'hourly',   domain:'inventory',  classification_level:'confidential',  schema_version:'v1' },
# //     { source_id:'src_iot_rfid_stream',    name:'IoT RFID Stream',    source_type:'streaming', ingestion_frequency:'real_time',domain:'logistics',  classification_level:'internal',      schema_version:'v3' },
# //     { source_id:'src_weather_api',        name:'Weather API',        source_type:'api',       ingestion_frequency:'hourly',   domain:'external',   classification_level:'public',        schema_version:'v1' },
# //     { source_id:'src_manufacturing_logs', name:'Manufacturing Logs', source_type:'file',      ingestion_frequency:'daily',    domain:'production', classification_level:'restricted',    schema_version:'v2' },
# //     { source_id:'src_legacy_trends',      name:'Legacy Trends',      source_type:'batch',     ingestion_frequency:'weekly',   domain:'analytics',  classification_level:'internal',      schema_version:'v1' },
# //   ];
# //   const stats=['completed','completed','completed','running','failed'];
...
#   const FLB={real_time:'Real-time',hourly:'Hourly',daily:'Daily',weekly:'Weekly',on_demand:'On demand'};
#   const ACT={real_time:100,hourly:80,daily:60,weekly:40,on_demand:20};
#   const filtered=F.cls!=='all'?sources.filter(s=>s.classification_level===F.cls)
#     :F.freq!=='all'?sources.filter(s=>s.ingestion_frequency===F.freq):sources;
#   const rows=document.getElementById('src-rows');
#   if(!filtered.length){rows.innerHTML='<div class="empty"><div class="ei">◎</div>No sources match filter</div>';return;}
#   rows.innerHTML=filtered.map((s,i)=>{
#     const col=COLS[i%COLS.length], act=ACT[s.ingestion_frequency]||20, cl=s.classification_level||'internal';
